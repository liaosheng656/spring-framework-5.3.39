...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	protected void publishEvent(Object event, @Nullable ResolvableType eventType) {
		...
		if (this.earlyApplicationEvents != null) {
            ...
            System.out.println("准备-----寻找合适的监听器接收事件，判断监听器event事件类型是否适配或符合");
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			try {
                ...
                //允许子类重写，在处理bean工厂后置处理器前，做点什么操作
                logger.info("postProcessBeanFactory(beanFactory),允许子类重写，在处理bean工厂后置处理器前，做点什么操作,Spring没有相关代码，给子类实现的");
				...
				registerListeners();
                System.out.println("=====注册事件监听器完成========");

                System.out.println("(最重要)初始化非懒加载的单例bean、bean的创建及生命周期、构造方法推断、循环依赖、" +
                        "bean的后置处理器、AOP、事务来了");
	...
	protected void finishBeanFactoryInitialization(ConfigurableListableBeanFactory beanFactory) {
        ...
        //如果没有默认的 embedded value resolver，加一个默认的解析器
        ...
        //bean的后置处理器、AOP、事务来了
        System.out.println("(最重要)初始化非懒加载的单例bean--准备开始");
	...
	protected void finishRefresh() {
        ...
        System.out.println("准备发布/推送容器刷新完成事件");
